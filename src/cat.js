		...
		for (let elem of this.elements) {
			...
			if (iterateVariable != null) {
				...
				for (let i = 0; i < iterateVariable.length; i++) { 
					this.append(elem, item.replace(/{{value}}/gi, iterateVariable[i]).replace(/{{i}}/gi, i));
				}
			}
		}
	},
			...
			if (iterateUrl != null) {
				...
				this.downloadJson("GET", iterateUrl, (json) => {
					for (let i = 0; i < json.length; i++) { 
						...
						while(inner.includes("{{") && inner.includes("}}")) {
							let expression = inner.split("{{")[1].split("}}")[0];
							let value = eval("json[i]." + expression);

							inner = inner.replace("{{" + expression + "}}", value);
						}

						this.append(elem, inner);
					}
				});
			}
		}
